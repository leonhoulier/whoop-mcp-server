                ...
                if expires_at:
                    try:
                        ...
                        if datetime.now(timezone.utc) >= exp_time:
                            ...
                            logger.info(f"✅ Tokens loaded from file (expires in {int(remaining)} seconds)")
...
def save_tokens(access: str, refresh: str, expires_in: int = None) -> None:
    ...
    if expires_in:
        logger.info(f"✅ Tokens saved securely (expires in {expires_in} seconds)")
    else:
        logger.info("✅ Tokens saved securely")



def save_tokens_from_response(token_data: dict) -> None:
    """Save tokens from WHOOP API response (includes expiration info)."""
    access = token_data.get("access_token")
    refresh = token_data.get("refresh_token", "")
    expires_in = token_data.get("expires_in", 3600)  # Default to 1 hour if not provided
    
    # Log what we receive
    logger.info(f"save_tokens_from_response called with keys: {list(token_data.keys())}")
    if not refresh:
        logger.warning("⚠️  No refresh_token in token_data - check WHOOP API response")
    else:
        logger.info(f"Refresh token received: {len(refresh)} characters")
...
async def refresh_access_token() -> bool:
    ...
    if not refresh_token:
        logger.warning("⚠️  No refresh token available - cannot refresh")
        return False
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(WHOOP_TOKEN_URL, data={
                ...
                "scope": "offline"  # Include scope parameter per WHOOP API spec
            })
            
            if response.status_code == 200:
                data = response.json()
                                # WHOOP API rotates refresh tokens - use new one if provided
                new_refresh = data.get("refresh_token", refresh_token)
                expires_in = data.get("expires_in", 3600)
                save_tokens(data["access_token"], new_refresh, expires_in)
                refresh_token = new_refresh  # Update global refresh token
                logger.info("✅ Access token refreshed successfully (with token rotation)")
                return True
            else:
...
async def ensure_valid_token() -> bool:
    """Check token expiration and refresh proactively if needed (within 5 minutes of expiration)."""
    global access_token
            ...
            with open(TOKEN_FILE, "r") as f:
                ...
                if expires_at:
                    ...
                    if time_until_expiry < 300:
                        logger.info(f"Token expires in {int(time_until_expiry)} seconds, refreshing proactively...")
