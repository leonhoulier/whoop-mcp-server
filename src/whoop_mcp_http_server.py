...
from whoop_mcp_server import server, load_config, load_tokens, save_tokens_from_response
...
async def handle_callback(request):
    """Handle OAuth callback from Whoop with authorization code and automatically exchange for tokens"""
    try:
            ...
            <html>
                <head>
                    ...
                    <style>
                        ...
                        .error {{ background-color: #fee; border-left: 4px solid #f00; padding: 15px; margin: 20px 0; }}
                        .btn {{ background-color: #2196F3; color: white; padding: 15px 32px; text-align: center; 
                ...
                <body>
                    ...
                    <a href="/reauth" class="btn">Try Again</a>
                </body>
            </html>
            """, status_code=400)
        ...
        if not code:
            ...
            <html>
                ...
                <body>
                    ...
                    <div class="error">
                        <p>No authorization code was provided in the callback URL.</p>
                    </div>
                    <a href="/reauth">Try Again</a>
                </body>
            </html>
            """, status_code=400)
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Access token present: {"access_token" in token_data}")
                    logger.info(f"Refresh token present: {"refresh_token" in token_data}")
                    if "refresh_token" in token_data:
                        refresh = token_data.get("refresh_token", "")
                        logger.info(f"Refresh token length: {len(refresh)} characters")
                        logger.info(f"Refresh token preview: {refresh[:20]}...")
                    logger.info(f"Expires in: {token_data.get("expires_in", "N/A")} seconds")
                    logger.info(f"Token type: {token_data.get("token_type", "N/A")}")
                    logger.info(f"Scope in response: {token_data.get("scope", "N/A")}")
                    
                    # Save tokens using the updated function
                    ...
                    <html>
                        <head>
                            ...
                            <style>
                                body {{ font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px; }}
                                .success {{ background-color: #efe; border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; }}
                                .info {{ background-color: #f0f8ff; border-left: 4px solid #2196F3; padding: 15px; margin: 20px 0; }}
                                .checkmark {{ color: #4CAF50; font-size: 48px; text-align: center; }}
                            </style>
                        </head>
                        <body>
                            ...
                            <div class="success">
                                <p><strong>Your Whoop account has been successfully connected!</strong></p>
                                <p>Tokens have been saved and your MCP server is now authenticated.</p>
                            </div>
                            <div class="info">
                                <p><strong>What happens next?</strong></p>
                                <ul>
                                    <li>Your access token will be automatically refreshed when it expires</li>
                                    <li>No further manual authentication should be required</li>
                                    <li>You can close this window and use the MCP server</li>
                                </ul>
                            </div>
                        </body>
                    </html>
            ...
            logger.error(f"Error exchanging tokens: {e}")
            return HTMLResponse(f"""
            <html>
                <head>
                    <title>Error</title>
